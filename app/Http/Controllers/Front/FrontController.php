...
class FrontController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $arr = ["d1"=>"data1","d2" => "data2", "d3" =>"data3"];
            // $arr["da"] = "data1";
    ...
    public function place_order(Request $request)
    {
        ...
        $arr=[
            ...
            "transacton_id"=>$request->payment_type,
        ...
        if($responseBody['code'] == 200){
            ...
            $txn_id= json_decode($responseBody)->body->transactionId;

            DB::table('orders')
            ->where(['id'=>$order_id])
            ->update(['txn_id'=>$txn_id, 'transaction_id'=>$txn_id]); 
    ...
    public function instamojo_payment_redirect(Request $request)
    {
        if($request->get('payment_id')!==null && $request->get('payment_status')!==null && $request->get('payment_request_id')!==null){
            if($request->get('payment_status')=='Credit'){
                $status='Success';
                $redirect_url= route('order_placed');
            }else{
                $status='Fail';
                $redirect_url= route('order_fail');
            }
